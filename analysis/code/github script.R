...
# available at https://github.com/cwlab23/lovingkindness/

df_wide <- read_csv("C:/Users/eliza/OneDrive/Desktop/Rutgers/Daily Compassion/Main study analysis/clean/JAMA/github/df_wide.csv")
df_long_ema <- read_csv("C:/Users/eliza/OneDrive/Desktop/Rutgers/Daily Compassion/Main study analysis/clean/JAMA/github/df_long_ema.csv")
df_long_training <- read_csv("C:/Users/eliza/OneDrive/Desktop/Rutgers/Daily Compassion/Main study analysis/clean/JAMA/github/df_long_training.csv")
...
df_wide %>% count(PID)  #confirm 390 participants randomized
...
df_wide <- df_wide %>%
  mutate(condition = recode(condition,
                            "A100" = "Other-focused Loving-Kindness",
                            "B100" = "Self-focused Loving-Kindness",
                            "C100" = "Active Control",
                            "D100" = "No-Intervention"))

df_wide %>% count(condition)
#Other-focused Loving-Kindness 99
#Self-focused Loving-Kindness 97
#Active Control 97
#No-Intervention 97
...
endpoint_attrition_by_condition <- df_wide %>%
  ...
  distinct(PID, condition) %>%
  count(condition, name = "n_missing")

endpoint_attrition_by_condition
#Other-focused Loving-Kindness 12
#Self-focused Loving-Kindness 7
#Active Control 15
#No-Intervention 18
...
EMA_excluded
#Other-focused Loving-Kindness 27
#Self-focused Loving-Kindness 33
#Active Control 32
...
#Other-focused Loving-Kindness 70
#Self-focused Loving-Kindness 63
#Active Control 58
#No-Intervention 79
...
df_long_ema_final <- df_long_ema %>%
  left_join(df_wide_final, by = "PID") # exclusion applied through left join by PID
...
df_long_training_final <- df_long_training_final %>%
  left_join(df_wide_final, by = "PID")
...
df_means <- df_long_ema_final %>%
  ...
  left_join(df_wide_final %>% dplyr::select(PID, age, gender, white, black, native, asian, pacific, latin,
...
df_long_training_final <- df_long_training_final %>%
  left_join(daily_dosage, by = c("PID", "condition", "date"))
...
model_well <- lmer(Well ~ condition * daily_dosage + 
                         age + education.years + political + gender + relationship + income + partyID +
                         (1 | PID),
                       data = df_long_training_final)
...
confint(model_well, method = "Wald")
...
confint(model_sleep_efficiency, method = "Wald")

#####positive mood

df_positive <- df_long_ema_final %>%
  dplyr::select(PID, day, condition, morning.positive, evening.positive,
                age, education.years, political, gender, relationship, income, partyID) %>%
...
df_negative <- df_long_ema_final %>%
  dplyr::select(PID, day, condition, morning.negative, evening.negative,
...
df_stress <- df_long_ema_final %>%
  dplyr::select(PID, day, condition, morning.stress, evening.stress,
...
df_dosage <- df_wide_final %>% filter(condition %in% c("Other-focused Loving-Kindness", "Self-focused Loving-Kindness"))
